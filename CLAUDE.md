...
4. Backend (NestJS + Prisma) Patterns
5. Frontend (Next.js App Router) Patterns
6. Database & Migrations (Prisma / Supabase)
## 4. Backend Patterns (NestJS + Prisma)

- Project structure: group by feature/module under `src/` (e.g., `src/users`, `src/orders`, `src/admin`).
- Controllers: thin, validate requests with DTOs and Zod pipes; return DTOs or primitives only.
- Services: implement business logic and all database access via a single `prisma` client instance.
- Modules: export providers and controllers; keep module boundaries clear and limited in size.
- DTOs: use `CreateXDto`, `UpdateXDto` naming; DTOs should mirror Zod schemas for runtime validation.
- Transactions: use `prisma.$transaction` for multi-step DB changes.
- Error handling: consistently throw Nest exceptions (`BadRequestException`, `NotFoundException`, etc.) and map Prisma errors where appropriate.
- Logging: use Nest `Logger` with structured messages and correlation IDs for requests.
- Health checks: expose `/api/health` with basic readiness and liveness checks.
- Global prefix: backend uses `app.setGlobalPrefix('api')` — ensure frontend `NEXT_PUBLIC_API_BASE` includes `/api` in dev.

Example controller pattern (conceptual):

1. Validate incoming request with a Zod schema / DTO.
2. Call service layer method.
3. Service performs Prisma operations and returns sanitized result.
4. Controller maps result to response DTO and returns.
